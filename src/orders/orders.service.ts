...
import { ORDERS_EXCHANGE, ORDERS_PROCESS_ROUTING_KEY } from 'src/rabbitmq/rabbitmq.constants';
...
export class OrdersService {
  ...
  async createOrder(userId: string, items: CreateOrderItemInput[]): Promise<Order> {
    ...
    const created = await this.dataSource.transaction(async (manager) => {
      ...
      const order = orderRepository.create({ userId: user.id, user, status: OrderStatus.PENDING });
